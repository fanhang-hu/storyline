...
- **理由 1：CPS 攻击具有物理后果。**
  - 论据1 ：CPS 将软件执行、传感器感知、控制决策和执行器动作连接到真实物理过程，并被广泛部署在 smart grid、industrial control systems、autonomous vehicles、medical devices 等安全关键场景中。因此，一旦攻击影响 sensing/control path，后果就可能从信息系统扩展到错误控制、服务中断、设备损坏甚至关键基础设施故障 [Humayed17; Mitchell14; Kayan22]。
...
- **理由 2：低延迟定位不足会削弱攻击响应和恢复决策。**
  - 论据1 ：已有 CPS IDS 研究指出，CPS 攻击既可能快速破坏物理过程，也可能像 Stuxnet 一样长期潜伏并逐步设置攻击条件，因此检测未知攻击和降低 detection latency 是 CPS IDS 的核心挑战。由于 CPS 控制闭环会把错误观测、异常控制命令或被篡改的运行时参数快速转化为 controller decision 和 actuator action，若检测主要停留在控制资产/工业协议状态或 sensor/control path 的物理后果上，系统可能要等到 residual、physical invariant 或受影响状态明显异常后才报警；此时响应时机可能已经晚于攻击从控制主机传播到 sensing/control path 的关键阶段，难以及时隔离触发攻击的恶意程序 [Mitchell14; Giraldo18; Hu18]。
  - 论据2 ：要实现这种定位，provenance evidence 能支持 attack reconstruction，但现有代表性工作通常只覆盖因果链的一侧。PLC-PROV 说明控制系统 provenance 可以解释 sensor、actuator 或 PLC 交互如何导致 CPS 后果；KAIROS 说明 whole-system provenance 可以重建 process、file、socket 层面的恶意活动。然而，前者不直接定位控制主机上的恶意程序 runtime 行为，后者不直接解释这些行为对应的 CPS physical consequence。因此，仍需要将两类证据连接起来，支持低延迟的攻击定位与响应 [PLCProv19; Kairos24]。
  - 论据3 ：这种定位不足还会影响后续 recovery decision。现有恢复决策通常依赖告警结果来选择恢复受影响的 sensor/control state、回滚异常操作，或重置受影响组件。Taser 和 RETRO 等入侵恢复研究进一步说明，恢复决策不能只关注系统是否回到某个正常状态，还需要区分 attack-dependent operations 与合法操作。若告警只能指出物理状态或组件异常，却不能定位触发攻击的恶意程序及其因果影响，后续恢复决策就可能只处理 sensor/control path 上的表面后果，而缺少面向攻击来源的处置依据；此时同一恶意程序仍可能继续触发攻击 [Taser05; RETRO10]。
...
- **背景知识 1.1**：本文关注的攻击路径
  - 解释：本文关注的是由控制主机、HMI、工程站、PLC 接口或 CPS 仿真/控制环境中的恶意程序触发的攻击。这类攻击通常需要通过文件、进程、socket、device interface、控制软件接口或工业通信端点与 sensing/control path 发生交互，因此会在主机侧留下可观测的 syscall/provenance 行为证据。对于完全不经过主机软件路径的物理扰动或硬件故障，本文将其视为由物理冗余、故障诊断或 safety monitoring 互补处理的场景。
  - 证据边界：本文的证据模型还假设审计日志或 provenance logs 由受保护的采集机制产生，攻击者可以通过恶意程序影响控制软件、文件、socket、device interface 或工业通信端点，但不能完全关闭、伪造或回滚日志采集通道本身。若攻击者已经控制日志基础设施或能够系统性篡改 provenance records，则需要结合可信执行、远程 attestation、secure logging 或独立监控通道来保证证据可信性，这属于互补防护假设，而不是本文主要解决的问题。
...
- **背景知识 2**：系统级行为证据与 provenance graph
  ...
  - 解释 2：这类行为证据可以被组织成 syscall trace，也可以进一步抽象为 provenance graph。在 provenance graph 中，process、file、socket、device interface 等系统实体作为节点，read、write、execute、send、receive 等交互作为因果边，从而描述攻击行为如何从 cyber layer 传播到 sensing/control path 和 physical process。

- **背景知识 3**：CPS attack attribution 与 recovery decision support
  - 解释 1：CPS attack attribution 不只是识别攻击类型，而是将异常与具体恶意程序、系统行为证据、受影响控制环节和物理后果之间的因果链关联起来。对本文而言，attribution 关注的是“哪个进程通过哪些系统交互影响了哪些 CPS 节点或组件”，而不是仅给出 attack/no-attack 判断。
  - 解释 2：CPS recovery decision support 指在攻击发生后，为系统恢复到安全、可控且不再受同一攻击源影响的状态提供证据依据。与普通状态恢复不同，面向攻击来源的恢复决策不仅需要了解 sensor、actuator 或 controller 的受影响状态，还需要识别触发攻击的恶意程序、相关系统行为证据和可能的攻击传播路径；否则恢复动作可能只覆盖物理状态或局部组件，而缺少判断攻击源是否已被清除的依据。
...
- **第一类方法：CPS/ICS 控制资产与控制链路防御**
  - 直接方法：面向 CPS 恶意程序和 ICS/PLC 攻击的防御方法，通常围绕控制系统资产、控制逻辑、工业协议和运行环境建立保护与检测机制。典型方法包括 defense-in-depth、网络分区与访问控制、工程站和 HMI 加固、PLC 程序完整性检查、远程 attestation、控制逻辑验证、工业协议监控、白名单执行、补丁管理，以及针对 PLC firmware、control logic、I/O、runtime memory 和 communication module 的异常检测 [Mitchell14; Kayan22; LopezMorales24]。这些方法分别关注控制资产是否可信、控制程序或配置是否被篡改、工业协议命令是否异常，以及控制设备运行状态是否偏离预期。
    - 桥接方法：PLC-PROV 这类面向控制系统的 provenance/forensics 方法可以被视为 CPS 防御与通用 provenance 方法之间的桥接。它们把 input/output、PLC interaction、sensor/actuator event 或 control-state change 组织成因果证据，用于 safety/security policy violation detection、attack reconstruction 和 impact understanding [PLCProv19]。
    - 方法的局限：这类方法能提供控制系统因果证据，但通常不直接建模恶意程序在控制主机上留下的细粒度 runtime 行为痕迹，也不一定能以低延迟把告警定位到具体进程及其系统级因果链。换言之，它们更擅长说明“控制链路或控制资产异常”，但较难说明“哪个进程通过哪些系统行为造成了这次 CPS 攻击”。
    - 局限的根本原因：CPS 防御长期以工业资产和控制链路为中心，观测对象多是 PLC 程序、工业协议、控制命令、HMI 显示、网络流量或控制事件；恶意程序在 OS 层面的 process、file、socket、syscall 行为没有被纳入统一的 CPS 攻击证据模型。因此，控制系统异常与恶意程序行为之间缺少可用于低延迟 attribution 和 recovery 的因果连接。
...
- **第二类方法：sensor/control path 数值异常检测**
  - 直接方法：面向传感器数值攻击的 CPS 检测方法，通常把攻击建模为 sensor reading、control input 或 physical state 与正常物理模型之间的偏离。典型检测方法包括 threshold-based detection、residual-based detection、Kalman filter / observer-based detection、chi-square detector、CUSUM / EWMA 等 stateful statistics、physical invariant checking、cross-sensor redundancy checking、watermarking 和 moving target defense [Giraldo18; Cardenas11; Mo15; Mitchell14]。这些方法通常通过比较观测值与预测值、检查物理不变量、累积 residual 变化或引入主动扰动来判断 sensor/control path 是否被篡改。
    ...
    - 局限的根本原因：传感器数值防御把攻击后果映射到物理量空间，依赖物理模型、统计阈值或传感器冗余来判断异常。它缺少从物理异常反向连接到 OS 层恶意程序 的因果证据，因此难以支持 process-level attribution 和面向攻击来源的 recovery。

- **第三类方法：host syscall/provenance 入侵检测**
  - 间接方法：非 CPS 场景下，针对 syscall trace 的恶意行为检测通常把程序运行时行为表示为系统调用序列或系统实体交互图。基于学习的方法使用 n-gram、frequency profile、TF-IDF、HMM、Markov model、SVM、LSTM、CNN、Transformer 或 NLP-style embedding 来学习正常 syscall sequence 与恶意 syscall sequence 的差异；基于规则的方法使用签名、if-then rules、RIPPER、automaton、专家规则或 CTI-derived graph pattern 匹配已知攻击行为；基于溯源图的方法将 process、file、socket 等实体构造成 provenance graph，通过因果依赖、信息流、图相似度、图嵌入或 GNN 检测 APT，并支持 attack reconstruction，例如 HOLMES、POIROT、UNICORN、KAIROS、ANUBIS、PROGRAPHER 等 [Forrest96; Warrender99; Liu18; Sworna23; Zipperle22; Li21; Holmes19; Poirot19; Unicorn20; Kairos24]。其中 KAIROS 属于 whole-system provenance IDS，能够从系统审计日志中检测异常事件并重建紧凑 attack footprint，因此更适合放在通用 host provenance 方法中，而不是放在 CPS/ICS 专用防御中。
    - 方法的局限：这类方法能较好刻画恶意程序的 OS-level 行为，但大多数不是为 CPS 场景设计。syscall-sequence 方法通常能判断进程行为是否异常，但缺少可解释的因果链；rule-based provenance 方法能匹配已知攻击模式，但依赖专家规则和先验攻击知识；learning-based provenance 方法能发现未知异常，但输出常是 anomalous node、time window 或 subgraph。部分在线 provenance investigation 系统已经能够恢复较紧凑的 attack-related provenance graph，从而提供细粒度 host-side attack footprint；但其输出通常面向通用主机入侵调查，不一定能把异常图、异常进程和异常分数组织成后续 CPS 影响分析可直接使用的 evidence boundary。
    - 方法局限的根本原因：syscall/provenance 方法的语义中心是计算系统中的 process-file-socket 交互，而 CPS 攻击的安全后果发生在 cyber-physical loop 中。如果缺少面向 CPS 影响分析的证据边界组织，系统级行为证据就难以直接转化为 CPS attack diagnosis、attribution 和 recovery decision 的入口信息。
...
- **共性缺陷 1：** 现有方法缺少以低延迟进程定位为主目标、并能为后续 CPS 控制/物理影响分析提供细粒度证据入口的 provenance 模型。
  ...
  - 技术原因 3：上述两类证据分别停留在 CPS 后果侧和 OS 行为侧，缺少统一的时间、实体和因果关联机制。结果是，CPS 侧告警往往低估攻击源定位问题，host 侧告警又难以说明物理影响；二者都不能同时满足低延迟检测、process-level attribution，以及为后续 CPS 节点影响分析和恢复决策提供细粒度证据入口的需求。
...
- **技术需求：** 需要从连续产生的 syscall/provenance stream 中，低延迟地捕捉短暂、局部且可能被正常行为稀释的恶意程序行为。同时，这些行为证据还需要保留明确的异常进程、异常分数和相关系统实体边界，为后续分析哪些 sensor、actuator、controller node、control command 或 physical/simulation component 可能受影响提供证据入口。
- **技术限制 1**：固定检测窗口难以同时覆盖攻击上下文和短攻击行为。
  - 解释：窗口过大时，短攻击行为会被大量正常 syscall/provenance events 稀释，导致异常分数下降或定位粒度变粗；窗口过小时，系统又可能丢失 process、file、socket 交互之间的因果上下文，难以判断局部异常是否属于同一恶意程序行为。因此，仅依赖固定窗口很难同时实现低延迟检测和细粒度定位。
- **技术需求矛盾 1**：系统既需要足够大的上下文来保持 provenance causality，又需要足够小的窗口来捕捉短攻击并定位可疑进程。
  - 解释：provenance-based detection 依赖 process、file、socket 和 syscall event 之间的因果关系，过度缩小窗口会破坏攻击链上下文；但 CPS 恶意程序可能只通过少量关键 syscall 或短步长行为触发 sensing/control path 上的影响，过大的窗口又会掩盖这些短暂异常。因此，核心矛盾在于如何自适应选择检测粒度，使系统既不丢失因果上下文，也不牺牲短攻击行为的可见性。
- **技术限制 2**：粗粒度 OS-level provenance evidence 难以作为后续 CPS 节点/组件影响分析的可靠入口。
  - 解释：process、file、socket、device interface 等系统实体通常只描述计算系统内部交互，而后续 CPS 影响分析需要从这些实体中识别与控制接口、通信端点、配置文件或 device interface 相关的可疑行为。如果告警只给出粗粒度异常窗口或单个异常节点，后续分析就缺少足够明确的进程边界和行为证据入口。
- **技术需求矛盾 2**：系统既需要保留足够细的系统级行为证据来支持 process-level attribution，又需要把证据组织成后续 CPS 节点影响分析和恢复决策能够使用的入口信息。
  - 解释：过细的 syscall/provenance 证据虽然有利于定位恶意进程，但如果缺少汇总，后续分析很难从大量底层事件中识别关键攻击行为；过粗的告警虽然便于后续处置，却会丢失攻击源、异常分数和相关系统实体边界。因此，挑战在于如何在不丢失攻击源定位能力的同时，把系统级因果证据汇总成可解释的细粒度证据入口，为后续 CPS 节点影响分析和恢复决策提供依据。
...
**Insight 1**：CPS 恶意程序的关键攻击行为在时间上可能短暂稀疏，但在因果上通常集中于少数 OS-level provenance evidence，因此检测粒度应围绕 evidence density 与 causality preservation 自适应变化，而不是固定观察整段执行。
- 该 insight 能解决现有方法的共性缺陷并克服相关困难的原因：固定窗口在 CPS 场景中存在天然矛盾：窗口过大时，短攻击行为会被大量正常 syscall/provenance events 稀释，导致异常证据密度下降或定位粒度变粗；窗口过小时，又会丢失 process、file、socket 之间的因果上下文，难以判断局部异常是否属于同一 恶意程序行为。围绕 evidence density 调整观察粒度，可以及时暴露可能传播到 sensing/control path 的短攻击证据；同时保留必要的 provenance causality，可以避免把孤立事件误判为攻击，并为后续 attribution 提供更准确的证据边界。

**Insight 2**：CPS 告警应给出可解释的 evidence boundary，作为后续 attribution、影响分析和恢复决策的入口。
- 该 insight 能解决现有方法的共性缺陷并克服相关困难的原因：通用 host provenance 方法可以恢复 host-side attack footprint，但其输出通常面向企业主机入侵调查，而不是 CPS sensing/control path 的后续影响分析。CPS 响应需要先知道异常是否由具体进程触发、异常证据集中在哪些系统实体上，以及这些实体是否涉及 file、socket、device interface 或 control-related resource。本文所需的 evidence boundary 至少包括：异常时间边界、异常进程节点、关键 syscall/provenance 子图、相关 file/socket/device/control resource，以及可交给后续 CPS 节点影响分析使用的接口。异常图、异常进程节点和异常分数共同形成这个边界，既比 attack/no-attack 或单个异常节点更细粒度，又不直接声称完成 recovery action；它为后续判断哪些 CPS 节点可能受到影响、以及如何进行恢复决策提供证据入口。
